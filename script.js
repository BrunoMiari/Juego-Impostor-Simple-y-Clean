...
function showScreen(screenId) {
    ...
    // DEBUG: Alertar si es la pantalla problemática
    if (screenId === 'liars-deck-interstitial-screen') {
        const checkEl = document.getElementById(screenId);
        if (!checkEl) console.error("CRITICAL: Screen element not found!");
    }

    document.querySelectorAll('.screen').forEach(screen => {
        screen.classList.remove('active');
        // Asegurar que se oculten incluso si tienen estilos inline
        if (screen.id !== screenId) screen.style.display = '';
    });
    ...
    if (target) {
        ...
        alert("Error: No se encontró la pantalla " + screenId);
    }
...
function showLiarsDeckSetup() {
    showScreen('liars-deck-setup-screen');
}

function showLiarsDeckNames() {
    const count = parseInt(document.getElementById('liars-player-count').value);
    if (count < 2 || count > 6) {
        alert("El número de jugadores debe ser entre 2 y 6");
        return;
    }

    const container = document.getElementById('liars-names-container');
    container.innerHTML = '';

    for (let i = 1; i <= count; i++) {
        container.innerHTML += `
            <div class="player-name-input">
                <input type="text" id="liar-player-${i}" placeholder="Jugador ${i}" value="Jugador ${i}">
            </div>
        `;
    }

    showScreen('liars-deck-names-screen');
}

function startLiarsDeckGame() {
    const count = parseInt(document.getElementById('liars-player-count').value);
    const names = [];
    for (let i = 1; i <= count; i++) {
        names.push(document.getElementById(`liar-player-${i}`).value);
    }

    try {
        if (window.LiarsDeck) {
            window.LiarsDeck.init(names);
        } else {
            alert("Error: El módulo Liar's Deck no se ha cargado correctamente.");
            console.error("window.LiarsDeck is undefined");
        }
    } catch (e) {
        alert("Error al iniciar Liar's Deck: " + e.message);
        console.error(e);
    }
}
...
function generateAdvancedHint(selectedWord, theme) {
    ...
    const advancedHints = {
        ...
        objetos: {
            ...
            // Ropa y Accesorios
            'Camisa': ['Botones', 'Pechera', 'Formal'],
            'Camiseta': ['Casual', 'Manga corta', 'Algodón'],
            'Pantalón': ['Piernas', 'Cintura', 'Bolsillos'],
            'Falda': ['Femenino', 'Abierta', 'Piernas'],
            'Vestido': ['Pieza única', 'Elegante', 'Cuerpo entero'],
            'Chaqueta': ['Abrigo ligero', 'Cierre o botones', 'Exterior'],
            'Abrigo': ['Invierno', 'Grueso', 'Protección frío'],
            'Suéter': ['Lana', 'Caliente', 'Tejido'],
            'Jersey': ['Deportivo', 'Equipo', 'Cómodo'],
            'Bufanda': ['Cuello', 'Lana', 'Invierno'],
            'Guantes': ['Manos', 'Dedos', 'Invierno'],
            'Gorro': ['Cabeza', 'Lana', 'Frío'],
            'Sombrero': ['Ala ancha', 'Sol', 'Estilo'],
            'Zapatos': ['Pies', 'Suela', 'Caminar'],
            'Botas': ['Altas', 'Resistentes', 'Invierno'],
            'Sandalias': ['Verano', 'Abiertas', 'Frescas'],
            'Zapatillas': ['Deportivas', 'Cómodas', 'Cordones'],
            'Calcetines': ['Pies', 'Algodón', 'Zapatos'],
            'Medias': ['Piernas', 'Nylon', 'Finas'],
            'Ropa Interior': ['Íntimo', 'Debajo', 'Diario'],
            'Pijama': ['Dormir', 'Noche', 'Cómodo'],
            'Bañador': ['Playa', 'Nadar', 'Agua'],
            'Bikini': ['Dos piezas', 'Playa', 'Mujer'],
            'Cinturón': ['Ajuste', 'Cintura', 'Hebilla'],
            'Corbata': ['Cuello', 'Nudo', 'Traje'],
            'Pajarita': ['Cuello', 'Moño', 'Elegante'],
            'Tirantes': ['Hombros', 'Sujeción', 'Pantalón'],
            'Bolso': ['Accesorios', 'Llevar', 'Mujer'],
            'Mochila': ['Espalda', 'Carga', 'Estudiante'],
            'Maleta': ['Viaje', 'Ruedas', 'Equipaje'],
            'Cartera': ['Dinero', 'Bolsillo', 'Tarjetas'],

            // Joyería
            'Collar': ['Cuello', 'Colgante', 'Adorno'],
            'Pulsera': ['Muñeca', 'Circular', 'Adorno'],
            'Anillo': ['Dedo', 'Compromiso', 'Circular'],
            'Pendientes': ['Orejas', 'Par', 'Adorno'],
            'Reloj': ['Muñeca', 'Tiempo', 'Accesorio'],
            'Broche': ['Ropa', 'Pin', 'Decorativo'],
            'Gemelos': ['Puños', 'Camisa', 'Elegante'],
            'Cadena': ['Metal', 'Eslabones', 'Cuello'],
            'Colgante': ['Cadena', 'Dije', 'Pecho'],
            'Diadema': ['Cabeza', 'Pelo', 'Adorno'],
            'Tiara': ['Princesa', 'Cabeza', 'Brillante'],
            'Corona': ['Rey', 'Cabeza', 'Poder'],

            // Herramientas
            'Martillo': ['Clavos', 'Golpear', 'Mango'],
            'Destornillador': ['Tornillos', 'Girar', 'Punta'],
            'Taladro': ['Agujeros', 'Eléctrico', 'Broca'],
            'Sierra': ['Cortar', 'Dientes', 'Madera'],
            'Lima': ['Suavizar', 'Metal', 'Aspereza'],
            'Llave Inglesa': ['Ajustable', 'Tuercas', 'Fontanería'],
            'Alicates': ['Sujetar', 'Cortar', 'Presión'],
            'Tijeras': ['Cortar', 'Dos hojas', 'Papel'],
            'Cutter': ['Hoja', 'Cortar', 'Afilado'],
            'Regla': ['Medir', 'Línea recta', 'Centímetros'],
            'Metro': ['Cinta', 'Medir', 'Enrollable'],
            'Nivel': ['Burbuja', 'Horizontal', 'Equilibrio'],
            'Tornillo': ['Espiral', 'Cabeza', 'Sujetar'],
            'Clavo': ['Punta', 'Metal', 'Martillo'],
            'Tuerca': ['Enroscar', 'Hexagonal', 'Perno'],
            'Arandela': ['Plana', 'Anillo', 'Tornillo'],
            'Pegamento': ['Unir', 'Adhesivo', 'Líquido'],
            'Cinta': ['Adhesiva', 'Pegar', 'Rollo'],

            // Oficina y Estudio
            'Libro': ['Lectura', 'Páginas', 'Escrito'],
            'Cuaderno': ['Escribir', 'Hojas', 'Notas'],
            'Libreta': ['Pequeña', 'Notas', 'Bolsillo'],
            'Agenda': ['Fechas', 'Organización', 'Días'],
            'Bolígrafo': ['Tinta', 'Escribir', 'Retráctil'],
            'Lápiz': ['Grafito', 'Borrable', 'Madera'],
            'Rotulador': ['Color', 'Grueso', 'Resaltar'],
            'Marcador': ['Resaltar', 'Fluorescente', 'Texto'],
            'Goma': ['Borrar', 'Lápiz', 'Corrección'],
            'Sacapuntas': ['Afilar', 'Lápiz', 'Viruta'],
            'Compás': ['Círculos', 'Dibujo', 'Punta'],
            'Calculadora': ['Números', 'Sumar', 'Pantalla'],
            'Grapadora': ['Unir', 'Papel', 'Metal'],
            'Perforadora': ['Agujeros', 'Papel', 'Archivar'],
            'Clips': ['Sujetar', 'Metal', 'Temporal'],
            'Chinchetas': ['Punta', 'Tablero', 'Fijar'],
            'Post-it': ['Adhesivo', 'Nota', 'Amarillo'],
            'Carpeta': ['Guardar', 'Documentos', 'Cartón'],
            'Archivador': ['Anillas', 'Organizar', 'Grande'],
            'Papel': ['Escribir', 'Blanco', 'Hoja'],
            'Sobre': ['Carta', 'Correo', 'Guardar'],
            'Sello': ['Correo', 'Postal', 'Estampilla'],
            'Impresora': ['Imprimir', 'Tinta', 'Papel'],
            'Escáner': ['Digitalizar', 'Copia', 'Imagen'],
            ...
            'Gafas de Sol': ['Protección UV', 'Oscuras', 'Verano'],
            'Lupa': ['Aumento', 'Detective', 'Cristal'],
            'Telescopio': ['Estrellas', 'Lejano', 'Astronomía'],
            'Microscopio': ['Diminuto', 'Laboratorio', 'Células'],
            'Brújula': ['Norte', 'Orientación', 'Magnética'],
            'Mapa': ['Geografía', 'Ubicación', 'Cartografía'],
            'GPS': ['Satélite', 'Ubicación', 'Ruta'],
            'Cerilla': ['Fuego', 'Madera', 'Encender'],
            'Encendedor': ['Fuego', 'Gas', 'Portátil'],
            'Candado': ['Seguridad', 'Llave', 'Cerrar'],
            'Cerradura': ['Puerta', 'Seguridad', 'Llave'],
            'Timbre': ['Sonido', 'Puerta', 'Aviso'],
            'Campana': ['Sonido fuerte', 'Bronce', 'Iglesia'],
            'Silbato': ['Sonido agudo', 'Árbitro', 'Alerta'],
            'Pelota': ['Juego', 'Redonda', 'Deporte'],
            'Muñeca': ['Juguete', 'Figura', 'Infantil'],
            'Peluche': ['Suave', 'Juguete', 'Abrazar'],
            'Puzzle': ['Piezas', 'Armar', 'Paciencia'],
            'Juego de Mesa': ['Fichas', 'Tablero', 'Social'],
            'Cartas': ['Baraja', 'Juego', 'Azar'],
            'Dados': ['Cubo', 'Números', 'Azar'],
            'Fichas': ['Juego', 'Redondas', 'Apuesta'],
            'Ajedrez': ['Estrategia', 'Tablero', 'Reyes'],
            'Damas': ['Fichas', 'Tablero', 'Diagonal'],
            'Dominó': ['Fichas', 'Puntos', 'Cadena'],

            // Jardín
            'Maceta': ['Planta', 'Barro', 'Tierra'],
            'Regadera': ['Agua', 'Plantas', 'Jardín'],
            'Manguera': ['Agua', 'Larga', 'Riego'],
            'Pala': ['Excavar', 'Tierra', 'Herramienta'],
            'Rastrillo': ['Hojas', 'Jardín', 'Limpieza'],
            'Tijeras de Podar': ['Cortar', 'Plantas', 'Jardín'],
            'Cortacésped': ['Hierba', 'Máquina', 'Jardín'],
            'Semillas': ['Plantar', 'Futuro', 'Vida'],
            'Abono': ['Nutrientes', 'Tierra', 'Crecimiento'],
            'Tierra': ['Suelo', 'Plantar', 'Marrón'],
            'Arena': ['Playa', 'Fina', 'Granos'],
            'Piedras': ['Duras', 'Mineral', 'Suelo']
        },
